> **REFRAMED DURING BRAINSTORM.** The original task #97 framing was "Modality: image/description (VLMs)" implying a new modality directory. The accepted design (Q1=A, Q3=A in this spec) reframed it as a *capability addition* to the existing `chat/completion` modality with new `supports_vision` and `supports_multi_image` capability flags. The "image/description" modality directory was never created. For current VLM architecture see `src/muse/modalities/chat_completion/runtimes/transformers_vlm.py` and the v0.42.0 release notes.
