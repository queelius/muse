...
`3d/generation` (v0.41.0+, extended in v0.43.0) hosts two runtimes under one MIME tag: `TripoSRRuntime` (image-to-3D, the original) and `ShapERuntime` (text-to-3D via `diffusers.ShapEPipeline`, added in v0.43.0). The 3D HF resolver dispatches per-family backend_path via `_runtime_path_for(repo_id)` in `model_3d_generation/hf.py`: repo names matching `shap-e` or `shape-e` route to `ShapERuntime`; all other repos (including the curated TRELLIS / Wonder3D / Hunyuan3D-2 entries) currently route to `TripoSRRuntime` as a fallback. Each future runtime release adds one constant plus one dispatch branch in `_runtime_path_for` plus one runtime file under `runtimes/`: Wonder3D arrives in v0.44.0, TRELLIS in v0.45.0, Hunyuan3D-2 in v0.46.0. `ShapERuntime` follows the deferred-imports pattern (module-level sentinels for `torch`, `ShapEPipeline`, `trimesh`; `_ensure_deps()` populates them lazily). Shap-E is text-only: the curated `shap-e` entry declares `supports_image_to_3d: False` and `supports_text_to_3d: True`; a request to `/v1/3d/from-image` with `model=shap-e` returns 400 before the runtime is invoked. GLB output flows through the existing codec: `trimesh.Trimesh` assembles vertices and faces from the pipeline's `.meshes[0]`, then `mesh.export(file_type="glb")` yields the bytes. The bundled `triposr` (Apache 2.0, ~1GB, image-to-3D) remains the default. A companion `_pip_extras_for(runtime_path)` helper mirrors the dispatch shape so each runtime's venv installs only the deps it needs.
