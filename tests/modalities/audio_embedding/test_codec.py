"""Tests for the audio/embedding codec.
...
from muse.modalities.audio_embedding.codec import (
    base64_to_embedding,
    embedding_to_base64,
)
...
def test_long_vector_roundtrip_preserves_dimension():
    """A 768-dim vector (MERT) survives encode/decode unchanged."""
...
def test_clap_512_dim_roundtrip():
    """A 512-dim vector (CLAP base) survives encode/decode unchanged."""
    original = [float(i) * 0.002 for i in range(512)]
    encoded = embedding_to_base64(original)
    decoded = base64_to_embedding(encoded)
...
def test_codec_module_reexports_helpers():
    ...
    from muse.modalities.audio_embedding import codec
